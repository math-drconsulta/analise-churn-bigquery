"""
Salva contratos individuais com score v4 (target real 30d) + faixas 5 e 7.
Inclui todas as features pra analise de grupos por faixa.
"""
import pandas as pd
import numpy as np
...
cols = [
    ...
    "churn_original", "churn_real_30d",
    "score_v4", "faixa_5", "faixa_7",
]
df[cols].to_csv("results/score_v4_contratos.csv", index=False)
print(f"Salvo: results/score_v4_contratos.csv ({len(df):,} contratos)")
print(f"Faixas 7: {df['faixa_7'].value_counts().to_dict()}")
